...
namespace zcf{
    std::string base64encode(const std::string& input){
        std::string result;
        result.resize(base64::encoded_size(input.length()));
        result.resize(base64::encode(&result[0],input.c_str(),input.length()));
    ...
    std::string base64decode(const std::string& input){
        std::string result;
        result.resize(base64::decoded_size(input.length()));
        result.resize(base64::decode(&result[0],input.c_str(),input.length()).first);
        return result;
    }
}//!namespace zcf